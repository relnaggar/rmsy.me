...
<form method="POST" action="{{ route('portal.buyers.update', $buyer) }}">
  ...
  <x-form-input name="id" label="ID" :value="$buyer->id" readonly />
  <x-form-input name="name" label="Name" :value="$buyer->name" required maxlength="255" />
  ...
  <x-form-input name="country" label="Country" type="select" :value="$buyer->country" :options="$countries" required />
