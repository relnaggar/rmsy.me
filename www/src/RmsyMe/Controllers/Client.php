...
class Client extends AbstractController
{
  ...
  public function welcomeSubmit(): Page
  {
    ...
    if (!isset($_FILES[$formName]) || !isset($_FILES[$formName]['error']['csvFile'])) {
    ...
    if ($_FILES[$formName]['error']['csvFile'] !== UPLOAD_ERR_OK) {
      $templateVars['errorCode'] = 'upload';
      // error_log('File upload error code: ' . $_FILES[$formName]['error']['csvFile']);
      return $this->getPage($templatePath, $templateVars);
    }

    // Validate file type (only allow CSV files)
    $finfo = new finfo(FILEINFO_MIME_TYPE);
    $mime = $finfo->file($_FILES[$formName]['tmp_name']['csvFile']);
    ...
    $destination = $uploadDir . basename($_FILES[$formName]['name']['csvFile']);
    if (!move_uploaded_file($_FILES[$formName]['tmp_name']['csvFile'], $destination)) {
      $templateVars['errorCode'] = 'upload';
