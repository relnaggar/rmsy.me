...
class Resume(models.Model, DocumentMixin):
  ...
  def __create(self, *args, **kwargs):
    if Resume.objects.filter(job=self.job).exists():
      self.version = Resume.get_next_version(self.job)
    else:
...
class ResumeSubstitution(models.Model):
  ...
  def __update(self, *args, **kwargs):
    super().save(*args, **kwargs)
    self.resume.generate_docx()
    self.resume.generate_png()

    if self.key not in self.resume.default_substitutions:
